...
func HelpScreen(width, height int) string {
	...
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("J/K") + descStyle.Render("Scroll right preview pane"))
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("pgup/pgdn") + descStyle.Render("Page scroll right preview pane"))
	content.WriteString("\n")

	// Actions
	content.WriteString(headerStyle.Render("Actions"))
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("enter/o") + descStyle.Render("Open file in external editor"))
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("E") + descStyle.Render("Edit selected file inline"))
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("O") + descStyle.Render("Open parent directory in editor"))
	content.WriteString("\n")
	content.WriteString(keyStyle.Render("e") + descStyle.Render("Edit selected file metadata"))
	content.WriteString("\n")
